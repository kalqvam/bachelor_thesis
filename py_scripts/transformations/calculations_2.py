...
class DataProcessor:
    ...
    def calculate_percentage_change(self, column: str, lag: int = 1) -> str:
        ...
        valid_time_cols = []
        if 'date' in self.df.columns:
            valid_time_cols.append('date')
        if 'quarter_year' in self.df.columns:
            valid_time_cols.append('quarter_year')
        if 'quarter' in self.df.columns:
            valid_time_cols.append('quarter')

        if not valid_time_cols:
            raise ValueError("The dataset must contain either 'date', 'quarter_year', or 'quarter' column for time-based calculations")

        time_col = valid_time_cols[0]
    ...
    def clean_dataset(self, keep_original: bool = False, drop_na: bool = True,
        ...
        if not keep_original:
            ...
            for transform in self.transformations:
                ...
                elif transform['type'] in ['percentage_change', 'normalized_values', 'natural_log', 'lag_difference', 'cross_sectional_ratio']:
                    columns_to_remove.append(transform['source_column'])
